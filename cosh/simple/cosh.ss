 ...
 (define (make-cosh-header depth)
   `(
    ...
    (define (variable app return)
     ...
     ;; "changed" is a property of stream elements. it is true if in
     ;; the computation of the current stream element any of the
     ;; stream elements taht were used differed from their previous
     ;; versions.

     (define (stream-converged? strm)
       (not (stream-changed? strm)))

     (define (stream-changed? strm)
       #t)

     ;; - stream-constant
     ;; - stream-cons (extra argument?)
     ;; how do recursive streams ever get marked as done?
     ;;
     ;; instead of done flag, have changed flag
     ;; stream has changed if any of its substreams changed
     ;; if we pull and it hasn't changed, we are done ... if it weren't for diagonal-policy (always pull in both directions; skip forward in return direction -- or could that cause problems?
     ;; maybe use first new element (s-car (s-cdr (fast-forward strm)))?
     ...
     (define-stream (stream-apply . maybe-strms)
       (letrec ([streams (map streamify maybe-strms)]
                [stream-of-return-streams (stream-map* dist-apply-to-stream streams)])
         (stream-let loop ([level 1]
                           [step 0]
                           [strms stream-of-return-streams])
                     (cond [(stream-null? strms) stream-null]
                           [(not (stream? (stream-car strms)))
                            (error 'stream-merge "non-stream object in input stream")]
                           [else
                            (let* ([local-output-stream (stream-car strms)]
                                   [input-done (stream-converged? strms)]
                                   [output-done (stream-converged? local-output-stream)])
                              (cond [(and input-done output-done) (stream-constant (stream-car local-output-stream))] ;; DONE
                                    [input-done (stream-cons (stream-car local-output-stream)
                                                             (loop level step (stream-constant (stream-cdr local-output-stream))))] ;; NOT DONE
                                    [output-done (stream-cons (stream-car local-output-stream)
                                                              (loop level step (stream-cdr strms)))] ;; NOT DONE
                                    [(stream-null? local-output-stream) (loop (+ level 1) 0 (stream-cdr strms))]
                                    [else
                                     (if (= step level)
                                         (stream-cons
                                          (stream-car local-output-stream)
                                          (loop (+ level 1) 0 (stream-cdr strms)))
                                         (stream-cons
                                          (stream-car local-output-stream)
                                          (loop level (+ step 1) (stream-cons (stream-cdr local-output-stream)
                                                                              (stream-cdr strms)))))]))]))))
